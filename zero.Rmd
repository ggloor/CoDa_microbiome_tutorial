...
# take the dataset, which is technical replicates of the
# yeast dataset and compare one technical replicate with the
# other

d <- read.table("data/ERP004763_sample_mapping.tsv", header=T, row.names=1)
...
In RNA-seq we typically do not have enough samples to have independent power for each feature.
 ...
 The data are probabilities scaled by the read count, then converted
 to integers by 'counting'
 if we sequenced 10 deeper, then 0 values would be converted to a
 continuum between 0 and ~10 (sampling), so what is an appropriate
 value for 0? somewhere between 0 and 1
 least likely value, over many experiments to perturb the system?
 0.5. Not always the best, usually not the optimum, but in general
 the least wrong
...
```
Lets generate 1000 random instances of the data. The maximum likelihood probability in the limit is the count divided by the sum of the sample. Since we know that a pure replicate is not identical, and differs by sampling variation we can estimate this as a multivariate Poisson process with a fixed sum: this is a Dirichlet distribution.
...
```{r, echo=T}
d.dir <- rdirichlet(1000, d.n0[,1]+0.5)
ml <- (d.n0[,1]+0.5) / sum(d.n0[,1]+0.5)
E_dir <- apply(d.dir, 2, mean)
plot(d.dir[1,], ml, pch=19, xlab="Dir", col=rgb(1,0,0,0.1), log="xy")
points(E_dir, ml, pch=19)
